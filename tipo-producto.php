    ...
    <div class="container">
        <?php
            if(isset($_GET['action']) == 'delete'){
                $id_delete = intval($_GET['ID_Tipo_Producto']);
                $query = mysqli_query($conn, "SELECT * FROM tipo_producto WHERE ID_Tipo_Producto='$id_delete'");
                if(mysqli_num_rows($query) == 0){
                    echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
                }else{
                    $delete = mysqli_query($conn, "DELETE FROM tipo_producto WHERE ID_Tipo_Producto='$id_delete'");
                    if($delete){
                        ...
                        Este color esta asociado con algunos datos ya registrados anteriormente</div>';
                    }
                }
            }
        ?>
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title" align="center"></i>Tipo de Producto</h3>
			</div>
			<div class="panel-body">
                ...
                <div class="pull-right">
				    <a href="registro-tipo-producto.php" class="btn btn-sm btn-primary" title="agregar un nuevo modelo"><span class="icon icon-plus" st></span><br>Nuevo Producto</a>
                </div><br><br>
                <hr>
        ...
        $(document).ready(function() {
				var dataTable = $('#lookup').DataTable( {
				 ...
				 "language":	{
					...
					"serverSide": true,
					"ajax":{
						url :"ajax-grid-data-tipo-producto.php", // json datasource

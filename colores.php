...
<script language="JavaScript">
    function pregunta(e){
      if (confirm('¿Estas seguro de que deseas eliminar este color?.')){
    ...
    <div class="container">
        <?php
            if(isset($_GET['action']) == 'delete'){
                $id_delete = intval($_GET['ID_Color']);
                $query = mysqli_query($conn, "SELECT * FROM colores WHERE ID_Color='$id_delete'");
                if(mysqli_num_rows($query) == 0){
                    echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
                }else{
                    $delete = mysqli_query($conn, "DELETE FROM colores WHERE ID_Color='$id_delete'");
                    if($delete){
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title" align="center"></i>Colores</h3>
			</div>
			<div class="panel-body">
                ...
                <div class="pull-right">
				    <a href="registro-color.php" class="btn btn-sm btn-primary" title="agregar un nuevo modelo"><span class="icon icon-plus" st></span><br>Nuevo Color</a>
                </div><br><br>
                <hr>
        ...
        $(document).ready(function() {
				var dataTable = $('#lookup').DataTable( {
				 ...
				 "language":	{
					...
					"serverSide": true,
					"ajax":{
						url :"ajax-grid-data-colores.php", // json datasource

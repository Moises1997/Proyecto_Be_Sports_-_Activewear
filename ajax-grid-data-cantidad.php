...
$query=mysqli_query($conn, $sql) or die("ajax-grid-data-cantidad.php: get InventoryItems");
...
if( !empty($requestData['search']['value']) ) {
	...
	$query=mysqli_query($conn, $sql) or die("ajax-grid-data-cantidad.php: get PO");
	$query=mysqli_query($conn, $sql) or die("ajax-grid-data-cantidad.php: get PO");
	$totalFiltered = mysqli_num_rows($query); // when there is a search parameter then we have to modify total number filtered rows as per search result without limit in the query

	$sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."   LIMIT ".$requestData['start']." ,".$requestData['length']."   "; // $requestData['order'][0]['column'] contains colmun index, $requestData['order'][0]['dir'] contains order such as asc/desc , $requestData['start'] contains start row number ,$requestData['length'] contains limit length.
	$query=mysqli_query($conn, $sql) or die("ajax-grid-data-cantidad.php: get PO"); // again run query with limit
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
    ...
    $nestedData[] = '<td><center>
                     <a href="editar_cantidad.php?ID_cantidad_modelo='.$row['ID_cantidad_modelo'].'"  data-toggle="tooltip" title="Editar datos" class="btn btn-sm btn-info"> <i class="menu-icon icon-pencil"></i> </a>
                     <a href="cantidad_modelo.php?action=delete&ID_cantidad_modelo='.$row['ID_cantidad_modelo'].'"  data-toggle="tooltip" title="Eliminar" class="btn btn-sm btn-danger" onclick="pregunta(event)"> <i class="menu-icon icon-trash"></i> </a>
				     </center></td>';

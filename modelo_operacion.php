...
<script language="JavaScript">
    function pregunta(e){
      if (confirm('¿Estas seguro de que deseas eliminar la operacion en este modelo?.')){
    ...
    <div class="container">
        <?php
            if(isset($_GET['action']) == 'delete'){
                $id_delete = intval($_GET['ID_Modelo_Operacion']);
                $query = mysqli_query($conn, "SELECT * FROM modelo_operacion WHERE ID_Modelo_Operacion='$id_delete'");
                if(mysqli_num_rows($query) == 0){
                    echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
                }else{
                    $delete = mysqli_query($conn, "DELETE FROM modelo_operacion WHERE ID_Modelo_Operacion='$id_delete'");
                    if($delete){
                        ...
                        echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Error, no se pudo eliminar los datos.<br>
                        Esta operacion esta asociada con algunos datos ya registrados anteriormente</div>';
                    }
                }
            }
        ?>
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title" align="center"></i>Operaciones por Modelo</h3>
			</div>
			<div class="panel-body">
                ...
                <div class="pull-right">
				    <a href="registro-modelo-operaciones.php" class="btn btn-sm btn-primary" title="agregar un nuevo modelo"><span class="icon icon-plus" st></span><br>Agregar Operaciones</a>
                </div><br><br>
                <hr>
        ...
        $(document).ready(function() {
				var dataTable = $('#lookup').DataTable( {
				 ...
				 "language":	{
					...
					"serverSide": true,
					"ajax":{
						url :"ajax-grid-data-modelo-operacion.php", // json datasource

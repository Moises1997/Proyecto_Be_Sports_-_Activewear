...
<script language="JavaScript">
      function pregunta(e){
          if (confirm('¿Estas seguro de que deseas eliminar esta operacion?.')){
    ...
    <div class="container">
        <?php
            if(isset($_GET['action']) == 'delete'){
                $id_delete = intval($_GET['ID_Operacion']);
                $query = mysqli_query($conn, "SELECT * FROM operaciones WHERE ID_Operacion='$id_delete'");
                if(mysqli_num_rows($query) == 0){
                    echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
                }else{
                    $delete = mysqli_query($conn, "DELETE FROM operaciones WHERE ID_Operacion='$id_delete'");
                    if($delete){
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title" align="center"></i> Operaciones</h3>
			</div>
			<div class="panel-body">
                <div class="pull-left">
                    <a href="menu_administrador.php" class="btn btn-sm btn-success"><span class="icon icon-home"></span><br>Menu Principal</a>
                </div>
                <div class="pull-right">
				    <a href="registro-operaciones.php" class="btn btn-sm btn-primary"><span class="icon icon-plus" st></span><br>Nueva Operación</a>
                </div><br><br>
                <hr>
        ...
        $(document).ready(function() {
				var dataTable = $('#lookup').DataTable( {
				 ...
				 "language":	{
					...
					"serverSide": true,
					"ajax":{
						url :"ajax-grid-data-operaciones.php", // json datasource

    ...
    <div class="container">
        <?php
            if(isset($_GET['action']) == 'delete'){
                $id_delete = intval($_GET['ID_Modelo']);
                $query = mysqli_query($conn, "SELECT * FROM modelo WHERE ID_Modelo='$id_delete'");
                if(mysqli_num_rows($query) == 0){
                    echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
                }else{
                    $delete = mysqli_query($conn, "DELETE FROM modelo WHERE ID_Modelo='$id_delete'");
                    if($delete){
        ...
        $(document).ready(function() {
				var dataTable = $('#lookup').DataTable( {
				 ...
				 "language":	{
					...
					"serverSide": true,
					"ajax":{
						url :"ajax-grid-data-modelos-vista.php", // json datasource
